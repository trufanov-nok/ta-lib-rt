...
use lib "../../../lib/perl";
use Test;
BEGIN { plan tests => 175, todo => [139, 153 .. 173] }
...
ok($TA_FUNC_UNST_EMA,1);
ok($TA_FUNC_UNST_HT_TRENDLINE,2);
ok($TA_FUNC_UNST_KAMA,3);
ok($TA_FUNC_UNST_MAMA,4);
ok($TA_FUNC_UNST_T3,5);
ok($TA_FUNC_UNST_ATR,6);
